...
class Multimodal_Data():
    ...
    def __init__(self,opt,tokenizer,dataset,mode='train',few_shot_index=0):
        ...
        # if mapping_word is None:
        # # Handle the case where label word is None
        # # Assign a unique token ID for the 'None' label
        #    self.label_mapping_id[label] = tokenizer._convert_token_to_id("<none>")
        #    print(f"Mapping for label {label}, word None, index {self.label_mapping_id[label]}")
        # else:
        # # Tokenize the label word and ensure there is exactly one token
        #    tokens = tokenizer.tokenize(' ' + mapping_word)
        
        # if len(tokens) == 1:
        #     self.label_mapping_id[label] = tokenizer._convert_token_to_id(tokens[0])
        #     print(f"Mapping for label {label}, word {mapping_word}, index {self.label_mapping_id[label]}")
        # else:
        #     # Handle the case where there is more than one token for the label word
        #     # You may want to adjust this based on how you want to handle multi-token labels
        #     print(f"Warning: Label word '{mapping_word}' tokenized into multiple tokens. Using the first token.")
        #     self.label_mapping_id[label] = tokenizer._convert_token_to_id(tokens[0])
        #     print(f"Mapping for label {label}, word {mapping_word}, index {self.label_mapping_id[label]}")
    ...
    def load_entries(self,mode):
        ...
        for k,row in enumerate(data):
            label=row['label']
            img=row['img']
            cap = captions.get(img.split('.')[0], '')
            if cap:
               cap = cap[:-1]#remove the punctuation in the end
            sent=row['clean_sent']
